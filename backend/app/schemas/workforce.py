...
class WorkerBulkImportRequest(BaseModel):
    workers: List[WorkerBulkImportItem] = Field(default_factory=list)
...
class WorkerBulkImportResult(BaseModel):
    ...
    createdWorkers: List[WorkerRead] = Field(default_factory=list)
...
class ShiftWorkerAssignRequest(BaseModel):
    workerIds: List[str] = Field(default_factory=list)
...
class ShiftCreate(ShiftBase):
    assignedWorkerIds: List[str] = Field(default_factory=list)
...
class ShiftRead(ShiftBase):
    ...
    assignedWorkers: List[WorkerShiftAssignmentRead] = Field(default_factory=list)
    createdAt: Optional[str] = None
...
class ShiftAttendanceComparisonRead(BaseModel):
    ...
    shiftName: str
    assignedTime: str
    actualCheckIn: Optional[str] = None
    actualCheckOut: Optional[str] = None
    assignedHours: float
    actualHours: float
    varianceHours: float
    status: str
...
class WorkforcePayrollBase(BaseModel):
    ...
    payrollStatus: Optional[str] = "Pending"
...
class WorkforceDashboardStats(BaseModel):
    ...
    onLeaveToday: int
    attendancePercentage: float
    categoryBreakdown: List[dict] = Field(default_factory=list)
    projectBreakdown: List[dict] = Field(default_factory=list)
    contractorBreakdown: List[dict] = Field(default_factory=list)
    recentAssignments: List[WorkerProjectAssignmentRead] = Field(default_factory=list)
