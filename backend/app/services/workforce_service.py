...
class WorkforceService:
    ...
    def verify_permission(
        ...
        """
        Validate workforce access based on user role,
        project assignment and contractor assignment.
        """
        ...
        if user_role == "Client":
            if not read_only:
                raise HTTPException(
                    ...
                    detail="Clients have read-only workforce access"
                )
            return True
        ...
        if user_role == "Contractor":
            if contractor_id and contractor_id != current_user.id:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail="Contractors can only access their own workforce"
                )

            # When a project is explicitly requested, ensure the contractor
            # actually has workers/assignments connected to that project.
            if project_id:
                linked = self.db.query(WorkerProjectAssignment).filter(
                    WorkerProjectAssignment.project_id == project_id,
                    WorkerProjectAssignment.contractor_id == current_user.id
                ).first()

                if not linked:
                    linked_worker = self.db.query(WorkerProjectAssignment).join(
                        Worker
                    ).filter(
                        WorkerProjectAssignment.project_id == project_id,
                        Worker.contractor_id == current_user.id
                    ).first()

                    if not linked_worker:
                        raise HTTPException(
                            status_code=status.HTTP_403_FORBIDDEN,
                            detail="You are not authorized for this project workforce"
                        )
        ...
        if user_role == "Project Manager":
            if project_id:
                ...
                if not project:
                    raise HTTPException(
                        status_code=status.HTTP_403_FORBIDDEN,
                        detail="You are not authorized to access this project workforce"
                    )
            elif not read_only:
                # Mutating operations must always identify a project.
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Project is required for this workforce operation"
                )
        ...
        if user_role == "Worker":
            if not read_only:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail="Workers cannot perform workforce management operations"
                )
            return True

        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You are not authorized to access workforce management"
        )
    ...
    def get_assignments(
        ...
        # Enforce project/contractor scope even when the frontend
        # does not send a filter.
        if role == "Contractor":
            effective_contractor = current_user.id
            query = query.filter(
                WorkerProjectAssignment.contractor_id == effective_contractor
            )
        elif role == "Project Manager" and not project_id:
            managed_projects = self.db.query(Project.id).filter(
                Project.project_manager_id == current_user.id
            ).subquery()
            query = query.filter(
                WorkerProjectAssignment.project_id.in_(managed_projects)
            )
        elif role == "Site Engineer" and not project_id:
            assigned_projects = self.db.query(ProjectSiteEngineer.project_id).filter(
                ProjectSiteEngineer.site_engineer_id == current_user.id
            ).subquery()
            query = query.filter(
                WorkerProjectAssignment.project_id.in_(assigned_projects)
            )
        ...
        if contractor_id:
            query = query.filter(
                WorkerProjectAssignment.contractor_id == contractor_id
            )
        if worker_id:
            query = query.filter(
        ...
        if assignment_status:
            query = query.filter(
                WorkerProjectAssignment.assignment_status == assignment_status
            )

        assignments = query.order_by(
            WorkerProjectAssignment.created_at.desc()
        ).all()
    ...
    def get_worker_assignment_history(
        ...
        query = self.db.query(WorkerProjectAssignment).filter(
            WorkerProjectAssignment.worker_id == worker_id
        )
    ...
    def get_shifts(
        ...
        if role == "Contractor":
            contractor_workers = self.db.query(Worker.id).filter(
                Worker.contractor_id == current_user.id
            ).subquery()
            contractor_shifts = self.db.query(
                WorkerShiftAssignment.shift_id
            ).filter(
                WorkerShiftAssignment.worker_id.in_(contractor_workers)
            ).subquery()
            query = query.filter(ShiftModel.id.in_(contractor_shifts))
        elif role == "Project Manager" and not project_id:
            managed_projects = self.db.query(Project.id).filter(
                Project.project_manager_id == current_user.id
            ).subquery()
            query = query.filter(ShiftModel.project_id.in_(managed_projects))
        elif role == "Site Engineer" and not project_id:
            assigned_projects = self.db.query(ProjectSiteEngineer.project_id).filter(
                ProjectSiteEngineer.site_engineer_id == current_user.id
            ).subquery()
            query = query.filter(ShiftModel.project_id.in_(assigned_projects))
        ...
        shifts = query.order_by(
            ShiftModel.date.desc(),
            ShiftModel.created_at.desc()
        ).all()
    ...
    def get_attendance(
        ...
        # Role-based scope when no explicit project filter is supplied.
        if role == "Contractor":
            query = query.join(AttendanceModel.worker).filter(
                Worker.contractor_id == current_user.id
            )
        elif role == "Project Manager" and not project_id:
            managed_projects = self.db.query(Project.id).filter(
            ...
            query = query.filter(
                AttendanceModel.project_id.in_(managed_projects)
            )
        elif role == "Site Engineer" and not project_id:
            assigned_projects = self.db.query(ProjectSiteEngineer.project_id).filter(
                ProjectSiteEngineer.site_engineer_id == current_user.id
            ...
            query = query.filter(
                AttendanceModel.project_id.in_(assigned_projects)
            )

        if contractor_id and role != "Contractor":
            query = query.join(AttendanceModel.worker).filter(
                Worker.contractor_id == contractor_id
            )

        if category_id:
            if role != "Contractor" and not contractor_id:
                query = query.join(AttendanceModel.worker)
            query = query.filter(
                Worker.workforce_category_id == category_id
            )

        records = query.order_by(
            AttendanceModel.date.desc(),
            AttendanceModel.created_at.desc()
        ).all()
    ...
    def get_attendance_summary(
        ...
        # Total workforce is based on workers assigned to the project,
        # while attendance counts come from the selected date.
        if project_id:
            worker_query = self.db.query(WorkerProjectAssignment.worker_id).filter(
                WorkerProjectAssignment.project_id == project_id,
                WorkerProjectAssignment.assignment_status == "Active"
            )
        elif role == "Contractor":
            worker_query = self.db.query(Worker.id).filter(
                Worker.contractor_id == current_user.id
            )
        elif role == "Project Manager":
            managed_projects = self.db.query(Project.id).filter(
            ...
            worker_query = self.db.query(
                WorkerProjectAssignment.worker_id
            ).filter(
                WorkerProjectAssignment.project_id.in_(managed_projects),
                WorkerProjectAssignment.assignment_status == "Active"
            )
        elif role == "Site Engineer":
            assigned_projects = self.db.query(
                ProjectSiteEngineer.project_id
            ).filter(
                ProjectSiteEngineer.site_engineer_id == current_user.id
            ).subquery()
            worker_query = self.db.query(
            ...
            worker_query = self.db.query(Worker.id)

        worker_ids = [row[0] for row in worker_query.distinct().all()]

        att_query = self.db.query(AttendanceModel)

        if worker_ids:
            att_query = att_query.filter(
                AttendanceModel.worker_id.in_(worker_ids)
            )
        else:
            att_query = att_query.filter(False)

        if project_id:
            att_query = att_query.filter(AttendanceModel.project_id == project_id)
        elif role == "Project Manager":
            managed_projects = self.db.query(Project.id).filter(
                Project.project_manager_id == current_user.id
            ).subquery()
        ...
        if contractor_id:
            att_query = att_query.join(AttendanceModel.worker).filter(
                Worker.contractor_id == contractor_id
            )

        if category_id:
            if not contractor_id:
                att_query = att_query.join(AttendanceModel.worker)
            att_query = att_query.filter(
                Worker.workforce_category_id == category_id
            )
        ...
        # One attendance record per worker/date is enforced by create_attendance,
        # so counts are directly usable here.
        present = sum(1 for r in records if r.status == "Present")
        absent = sum(1 for r in records if r.status == "Absent")
        leave = sum(1 for r in records if r.status == "Leave")

        # The denominator for the dashboard is the authorized workforce,
        # not merely workers who already have an attendance record.
        total_workers = len(worker_ids)
        attendance_pct = (
            round((present / total_workers) * 100.0, 1)
            if total_workers > 0
            else 0.0
        )
    ...
    def get_dashboard_stats(
        ...
        att_pct = round((present_today / total_workers * 100.0), 1) if total_workers > 0 else 0.0
